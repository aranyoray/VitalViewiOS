...
final class MockSource: BaseDataSource {
    ...
    private var _deviceName: String? = "VitalView Mock"
    ...
    private let queue = DispatchQueue(label: "com.vitalview.mocksource")
    ...
    func setParams(_ mutate: (inout MockParams) -> Void) {
        mutate(&signal.p)
    }

    func getParams() -> MockParams { signal.p }

    /// Inject a motion burst now (drives the motion/gating demo).
    func injectMotion(durationMs: Double = 2000, severity: Double = 4) {
        let now = deviceNowUs()
        signal.p.motionBursts.append(MotionBurst(startUs: now, endUs: now + durationMs * 1000, severity: severity))
    }

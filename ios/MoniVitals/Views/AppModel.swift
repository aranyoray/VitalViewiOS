...
final class AppModel: ObservableObject {
    ...
    /// so its parameters are NOT adjustable and those controls must stay hidden.
    @Published private(set) var isTunableSignal = false
    ...
    // MARK: - Real-recording reference (when replaying bundled data)
    @Published private(set) var referenceHR: Double?
    @Published private(set) var referenceSpO2: Double?
    @Published private(set) var recordingName: String?
    /// Fixed replay rate (Hz) when playing a recording; nil when tunable (mock) source is active.
    @Published private(set) var replayFs: Int?
    ...
    private let settingsKey = "monivitals.settings"

    private static let tickInterval = 0.25

    /// Human-readable identity of the on-device inference model backing the metrics.
    var modelName: String { engine.name }

    init() {
        settings = AppModel.loadSettings(key: "monivitals.settings")
    ...
    func updateSettings(_ mutate: (inout AppSettings) -> Void) {
        ...
        // Push rate changes to the active source.
        if let src = source {
    ...
    /// Start the standalone runtime on launch: open the bundled recording and begin
    /// streaming automatically once the replay source is live. Idempotent.
    func begin() {
    ...
    /// Replay the bundled REAL recording (PhysioNet BIDMC ECG + PPG) through the DSP.
    func connectReplay() {
        teardownSource()
        guard let src = ReplaySource.make() else {
            // Bundled recording missing/unreadable — stay disconnected rather than crash.
            source = nil
    ...
    /// Effective sample rate (Hz) for a stream: the recording's fixed rate while replaying,
    /// otherwise the configured stream rate (mock honours per-stream rates).
    func sampleRateHz(_ stream: StreamKind) -> Int {
        replayFs ?? settings.streamRates[stream] ?? BLEProtocol.defaultRateHz(stream)
    }
    ...
    func startRecording(label: SessionLabel, notes: String) {
        ...
        let session = Session(
            ...
            firmwareVersion: recordingName != nil ? "bidmc-replay" : (isTunableSignal ? "1.0.0-mock" : "unknown"),
    ...
    private func wire(_ src: DataSource) {
        ...
        ev.connection
            ...
            .sink { [weak self] state, _ in
                ...
                if state == .connected && self.autoStream && !self.streaming { self.startStreaming() }
            }
            .store(in: &cancellables)
    }
    ...
    private func tick() {
        ...
        let app = engine.infer(at: latest)
